...
const membersUpdate = async (req, res) => {
  const { email, nickname, roleType, password, cfm_password } = req.body;
  const user_id = req.params._id;
  const userInfo = await User.findOne({ _id: user_id });
  const hash = await bcrypt.hash(password, 12);

  const errors = {};
  const values = { email, nickname, roleType, password, cfm_password };
  adminMembersValidator(errors, values);

  if (!(Object.keys(errors).length === 0)) {
    // 템플릿 문자열 `admin/members/update/${user_id}`
    return res.render('admin/members/update', {
      userInfo,
      errors,
      values,
    });
